...
// Chunked transfer-encoding, HTTP/1.1 (RFC 7230 §4.1). No coroutines: the
// Lua handler is still one synchronous call that invokes writeChunk()
// zero or more times before returning; each call is a real, immediate
// socket write+flush using the same req.writer.interface every buffered
// response already writes through. Backpressure comes from the same place
// buffered responses already get it: flush() blocks on this connection's
...
pub fn writeChunk(req: *Request, chunk: []const u8) !void {
    ...
    var size_buffer: [18]u8 = undefined; // 16 hex digits + \r\n
