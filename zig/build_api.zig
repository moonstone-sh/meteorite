...
pub const Options = struct {
    meteorite_root: []const u8,
    target: std.Build.ResolvedTarget,
    optimize: std.builtin.OptimizeMode,
    mode: []const u8 = "release-static",
    ...
    fast_http_workers: u16 = 0,
    fast_http_queue: u16 = 1024,
    benchmark_instrumentation: bool = false,
    unix_socket_path: []const u8 = "/tmp/meteorite.sock",
    unix_socket_mode: []const u8 = "0660",
    unix_socket_unlink_stale: bool = true,
...
fn meteoritePath(b: *std.Build, meteorite_root: []const u8, relative_path: []const u8) std.Build.LazyPath {
    return cwdPath(join(b, &.{ meteorite_root, relative_path }));
}
...
fn configureNativeModule(b: *std.Build, module: *std.Build.Module, native: NativeOptions, project_root: []const u8) void {
    for (native.modules) |entry| module.addImport(entry.name, entry.module);
    for (native.include_paths) |include_path| {
        module.addIncludePath(cwdPath(projectPath(b, project_root, include_path)));
    }
}

fn addZigFileImports(b: *std.Build, graph_module: *std.Build.Module, graph_output: []const u8, target: std.Build.ResolvedTarget, optimize: std.builtin.OptimizeMode, native: NativeOptions, project_root: []const u8) void {
    ...
    while (lines.next()) |line| {
        ...
        configureNativeModule(b, module, native, project_root);
        graph_module.addImport(name, module);
    }
}

pub fn addService(b: *std.Build, options: Options) Service {
    ...
    const hybrid_profile = options.hybrid_profile;
    const backend = options.backend;
    const fast_http_strategy = options.fast_http_strategy;
    const fast_http_workers = options.fast_http_workers;
    const fast_http_queue = options.fast_http_queue;
    const benchmark_instrumentation = options.benchmark_instrumentation;
    const unix_socket_path = options.unix_socket_path;
    const unix_socket_mode = options.unix_socket_mode;
    const unix_socket_unlink_stale = options.unix_socket_unlink_stale;
    const require_peer_credentials = options.require_peer_credentials;
    ...
    const pattern_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, "zig/pattern.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const handlers_module = b.createModule(.{
        .root_source_file = if (cwdFileExists(b, project_handlers)) cwdPath(project_handlers) else meteoritePath(b, meteorite_root, "zig/empty_handlers.zig"),
        .target = target,
        .optimize = optimize,
    });
    handlers_module.addImport("meteorite_build_info", build_info_module);
    configureNativeModule(b, handlers_module, options.native, project_root);
    const project_validators = join(b, &.{ project_root, "zig/validators.zig" });
    const validators_module = b.createModule(.{
        .root_source_file = if (cwdFileExists(b, project_validators)) cwdPath(project_validators) else meteoritePath(b, meteorite_root, "zig/empty_validators.zig"),
        .target = target,
        .optimize = optimize,
    });
    configureNativeModule(b, validators_module, options.native, project_root);
    const protocol_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, "zig/backends/protocol.zig"),
        .target = target,
        .optimize = optimize,
    });
    ctx_module.addImport("meteorite_protocol", protocol_module);
    const http_date_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, "zig/server/http_date.zig"),
        .target = target,
        .optimize = optimize,
    });
    const signals_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, "zig/server/signals.zig"),
        .target = target,
        .optimize = optimize,
    });
    const cached_time_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, "zig/server/cached_time.zig"),
        .target = target,
        .optimize = optimize,
    });
    const c_imports_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, "zig/bridge/c_imports.zig"),
        .target = target,
        .optimize = optimize,
    });
    const bridge_stats_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, "zig/bridge/lua_stats.zig"),
        .target = target,
        .optimize = optimize,
    });
    const bridge_bench_stats_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, if (benchmark_instrumentation) "zig/bridge/lua_bench_stats.zig" else "zig/bridge/lua_bench_stats_stub.zig"),
        .target = target,
        .optimize = optimize,
    });
    const bridge_vtable_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, "zig/bridge/lua_vtable.zig"),
        .target = target,
        .optimize = optimize,
    });
    bridge_vtable_module.addImport("meteorite_protocol", protocol_module);
    const bridge_json_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, "zig/bridge/lua_json.zig"),
        .target = target,
        .optimize = optimize,
    });
    bridge_json_module.addImport("c_imports", c_imports_module);
    const bridge_http_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, "zig/bridge/lua_http.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const meteorite_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, "zig/meteorite.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const server_validators_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, "zig/server/validators.zig"),
        .target = target,
        .optimize = optimize,
    });
    const server_static_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, "zig/server/static_files.zig"),
        .target = target,
        .optimize = optimize,
    });
    const server_limits_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, "zig/server/request_limits.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const bridge_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, "zig/bridge.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    configureNativeModule(b, graph_module, options.native, project_root);

    const bridge_bindings_module = b.createModule(.{
        .root_source_file = meteoritePath(b, meteorite_root, "zig/bridge/lua_bindings.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    addZigFileImports(b, graph_module, project_graph_output, target, optimize, options.native, project_root);
    ...
    const exe = b.addExecutable(.{
        ...
        .root_module = b.createModule(.{
            .root_source_file = meteoritePath(b, meteorite_root, "zig/main.zig"),
            .target = target,
            .optimize = optimize,
            .imports = &.{
    ...
    for (options.native.libraries) |library| exe.root_module.linkLibrary(library);
    for (options.native.system_libraries) |library_name| exe.root_module.linkSystemLibrary(library_name, .{});
    if (options.native.c_sources.len > 0) {
        exe.root_module.addCSourceFiles(.{
            .root = cwdPath(project_root),
            .files = options.native.c_sources,
            .flags = options.native.c_flags,
        });
    }
    for (options.native.include_paths) |include_path| {
        exe.root_module.addIncludePath(cwdPath(projectPath(b, project_root, include_path)));
    }
    ...
    return .{ .executable = exe, .install_step = install_step, .run_step = run_step };
}
