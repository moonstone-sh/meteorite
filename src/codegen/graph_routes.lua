...
function graph_routes.route_handler_zig(route)
  ...
  if route.handler.kind == "lua" then handler = ".{ .lua_file = .{ .id = " .. helpers.zig_string(route.id) .. ", .path = " .. helpers.zig_string(route.handler.path or route.handler.module) .. ", .nparams = " .. tostring(route.handler.nparams or 1) .. ", .arg_mode = ." .. tostring(route.handler.arg_mode or "request_table") .. " } }" end
...
function graph_routes.plugin_handler_zig(plugin)
  ...
  elseif handler.kind == "lua" then
    return ".{ .lua_file = .{ .id = " .. helpers.zig_string(plugin.id) .. ", .path = " .. helpers.zig_string(handler.path or handler.module) .. ", .nparams = " .. tostring(handler.nparams or 1) .. ", .arg_mode = ." .. tostring(handler.arg_mode or "request_table") .. " } }"
